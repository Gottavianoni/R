#Lecture et chargement du robot_tra
#Stockdepot
library(readxl)
chemin <- "O:/01-MAINTENANCE SI/ODS STOCK/stock_ods/robot_tra/"
...
type <- substr(tolower(fichiers),regexpr("(st+|exp+|tra+)",tolower(fichiers),perl =T),regexpr("(st+|exp+|tra+)",tolower(fichiers),perl=T)+2)
...
for (file in xls) {
  ...
  #Contrôle des fichiers mauvaise feuille
