...
for( file in files_stk) {
  ...
  ls_fil <-   c("006","010","014","024","025","026","027","029","030","034","049","053","061","064","066","067","070","071","073","079","081","301","303","305","306","308","312")
...
for( file in files_stk) {
  ...
  if (nchar(ind1) != 6) {
  ind1 <- as.numeric(paste0(substr(Sys.Date(),1,4),substr(Sys.Date(),6,7))) - 1
...
for(file in files_stk){
  ...
  if(length(unique(paste0(df$V1,df$V2,df$V3,df$V4,df$V5,df$V7,df$V8))) !=  length(unique(paste0(df$V1,df$V2,df$V3,df$V4,df$V5)))){
  df1 <- aggregate(x = df[,c(6,8,10:17)] , by = df[,c(1:4,9)], FUN = sum)
  df2 <- aggregate(x = df[,5] , by = df[,c(1:4,9)], FUN = paste0)
  chaain <- NULL
  for (i in 1:length(df2$x)) chaain <- c(chaain,df2$x[[i]][[1]])
  df1$V7 <- df1$V8/df1$V6
  df1$V5 <- chaain
  df <- df1[,c(1:4,17,6,16,7,5,8,9,10:15)]
  }
  assign(file,df)
}
